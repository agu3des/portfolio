...
class HashTable:
    def __init__(self, size:int=10):
        ...
        '''
        self.__used = 0 #como é dispersa, se n tiver contador tem que varrer toda vez
    ...
    def remove(self, key:any)->any:
        ...
        slot = self.__hash(key)
        for i in range(len(self.__table[slot])):
            if key == self.__table[slot][i].key: #se a chave na tabela, passando o slot do i como chave
                return self.__table[slot].pop(i).value #pop é um método de remoção de List, passando sua posição
        else:
            raise KeyError(f'key {key} not found')
